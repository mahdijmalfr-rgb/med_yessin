...
txt = Text(root, width=50, height=4, bg="black", fg="white", font=("Arial", 11))
class SmartAnalyticBot:
    ...
    def fetch_internet_info(self, query):
        ...
        try:
            ...
            return f"حدث خطأ: {e}"
        reponse += (get_wiki_summary("Tunis"))

    def process_message(self, raw_input):
        reponse=""
        """الدالة الرئيسية التي تدير المحادثة بالكامل (Chat Loop)"""
        reponse +=("=" * 50)
        reponse +=(f"{self.bot_name}:Welcome to the organized version with external memory!")
        reponse +=("=" * 50)
        
        # استقبال النص
        raw_input=txt.get("1.0","end-1c").strip()
...
if __name__ == "__main__":
    ...
    # شاشة العرض
    lbe1 = Label(root, text="أهلاً بك! اكتب رسالتك واضغط Enter.", width=80, height=30, bg="black", fg="white", font=("Arial", 11), wraplength=350)
    lbe1.pack(pady=10)
    ...
    # دالة زر Enter
    def on_enter(event):
        user_input = txt.get("1.0", "end-1c").strip()
        if user_input:
            result = bot.process_message(user_input)
            lbe1["text"] = result  # عرض النتيجة
            txt.delete("1.0", END) # مسح مربع الإدخال
        return "break"  # منع نزول سطر جديد
